...
router_addProduct.post('/api/addProduct', async (req, res) => {
    ...
    try {
      const { Username, Password, Name, Description, Cost, Category,Picture } = req.body;
      ...
      const query = "INSERT INTO products (Name, Description, Cost, ShopID ,Category,Picture) VALUES (?, ?, ?, ?, ?,?) ";
      const values = [Name,Description,Cost,shopID,Category, Picture];

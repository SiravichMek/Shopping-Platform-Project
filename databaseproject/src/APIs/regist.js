...
router_regist.post('/api/createUser', async (req, res) => {
  const { Name, Username, Password, confirmpassword, Address, Tel,Image } = req.body;
  ...
  if (Password !== confirmpassword) {
    ...
    const query = 'INSERT INTO users (Name, Username, Password, Tel, Address,Image) VALUES (?, ?, AES_ENCRYPT(?, SHA1(?)), ?,?,?)';
    const values = [Name, Username, Password,'Password', Tel, Address,Image];
...
const getUserID = async (Username, Password) => {
  const db = await createConnection();
  const query = 'SELECT UserID FROM users WHERE Username = ? AND Password = AES_ENCRYPT(?, SHA1(?))';
